package CrazyJar.lib;

/**
 * Created by yuany on 2016/6/29.
 */
public class Reference {

    //MOD信息
    public static final String MOD_ID = "CrazyJar";
    public static final String MOD_PREFIX = "CrazyJar.";
    public static final String MOD_NAME = "CrazyJar";
    public static final String VERSION = "alpha 0.1";
    public static final String proxy_loc = "CrazyJar.proxy.";
    //文件路径
    public static final String MODEL_PREFIX = MOD_ID + ":textures/models/";
    public static final String TEXTURE_PREFIX = MOD_ID + ":";
    //贴图
    public static final String MODEL_ESSENTIA_Jar1_TEXTURE = MODEL_PREFIX + "essentiaJar1.png";
    public static final String MODEL_ESSENTIA_Jar2_TEXTURE = MODEL_PREFIX + "essentiaJar2.png";
    public static final String MODEL_ESSENTIA_Jar3_TEXTURE = MODEL_PREFIX + "essentiaJar3.png";
    public static final String MODEL_ESSENTIA_Jar4_TEXTURE = MODEL_PREFIX + "essentiaJar4.png";
    public static final String MODEL_ESSENTIA_Jar5_TEXTURE = MODEL_PREFIX + "essentiaJar5.png";
}
